    ...
    d3.tsv("data/Huffington.tsv", function(error, data_) {
        ...
        for (var i=0;i<links2.length;i++){
            ...
            for (var m=0;m<numMonth;m++){
                if (links[name1+"__"+name2][m]){
                    if (links[name1+"__"+name2][m]>valueSlider) //links[name1+"__"+name2][m]>ccc &&
                        ccc+=links[name1+"__"+name2][m];
                }
            }
            links2[i].count = ccc;
        }
                ...
                if (selectedTerms[term1]){   // if the term is in the selected 100 terms
                    for (var term2 in d) {
                        if (selectedTerms[term2]){   // if the term is in the selected 100 terms
                            if (!links[term1+"__"+term2]){
                                links[term1+"__"+term2] = new Object();
                                links[term1+"__"+term2].max = 1;
                                links[term1+"__"+term2].maxMonth =m;
                            }
                            if (!links[term1+"__"+term2][m])
                                links[term1+"__"+term2][m] = 1;  // in month m
                            else{
                                links[term1+"__"+term2][m]++;// add 1 more if occur more than 1 in a timestep
                                if (links[term1+"__"+term2][m]>links[term1+"__"+term2].max){
                                    links[term1+"__"+term2].max = links[term1+"__"+term2][m];
                                    links[term1+"__"+term2].maxMonth =m;

                                    // max of a links: max number of co-occurrence in a timestep
                                    // maxMonth: timestep that has max
                                    if (links[term1+"__"+term2].max>relationshipMaxMax) // max over time
                                        relationshipMaxMax = links[term1+"__"+term2].max;
                                }
                            }
                        }
                    }
...
function computeConnectivity(a, num) {
    ...
    for (var i=0; i<num;i++){
        ...
        for (var j=i+1; j<num;j++){
            var term2 =  a[j].term;
            if (links[term1+"__"+term2] && links[term1+"__"+term2].max>=valueSlider){
                if (links[term1+"__"+term2].max> a[i].isConnected){
                    a[i].isConnected = links[term1+"__"+term2].max;
                    a[i].isConnectedMaxMonth = links[term1+"__"+term2].maxMonth;
                }
                if (links[term1+"__"+term2].max> a[j].isConnected){
                    a[j].isConnected = links[term1+"__"+term2].max;
                    a[j].isConnectedMaxMonth = links[term1+"__"+term2].maxMonth;
                }
            }
            // isConnected: for each term, equals to max of all links

            else if (links[term2+"__"+term1] && links[term2+"__"+term1].max>=valueSlider){
                if (links[term2+"__"+term1].max>a[i].isConnected){
                    a[i].isConnected = links[term2+"__"+term1].max;
                    a[i].isConnectedMaxMonth = links[term1+"__"+term2].maxMonth;
                }
                if (links[term2+"__"+term1].max>a[j].isConnected){
                    a[j].isConnected = links[term2+"__"+term1].max;
                    a[j].isConnectedMaxMonth = links[term1+"__"+term2].maxMonth;
                }
            }
            //if (term2=="beijing")
...
function computeLinks() {
        ...
        for (var j=i+1; j<numNode;j++){
            ...
            if (links[term1+"__"+term2] && links[term1+"__"+term2].max>=valueSlider){
                for (var m=1; m<numMonth;m++){
                    if (links[term1+"__"+term2][m] && links[term1+"__"+term2][m]>=valueSlider){
                        var sourceNodeId = i;
                        var targetNodeId = j;
                        ...
                        links.push(l);
                        if (links[term1+"__"+term2][m] > relationshipMaxMax2)
                            relationshipMaxMax2 = links[term1+"__"+term2][m];
                    }
                }
            }
        }
    ...
    links.forEach(function(l) {
        ...
        var month = l.m;
        l.weight = links[term1+"__"+term2][month];
        l.value = linkScale(links[term1+"__"+term2][month]);
    }  );
